#!/usr/bin/env python3
"""Generate Phase 3 Activity 1 — Swin UNETR Training notebook (.ipynb)."""
import json, sys
...
# Cell 0: Title
# ════════════════════════════════════════════════════════════
md("""# Phase 3 — Swin UNETR: Vision Transformer Baseline
## Fine-tuning BraTS-2021 Pretrained Encoder on Cyprus PROTEAS

**Paper:** Hatamizadeh et al. (2022) – Swin UNETR, BrainLes@MICCAI

**Key Advantages over Met-Seg CNN:**
- **Global attention** via shifted windows (vs 3×3×3 local convolutions)
- **Hierarchical multi-scale features** (48→96→192→384→768 channels)
- Pretrained on **5,050 CT + 1,251 BraTS MRI** volumes
- **768-dim bottleneck embeddings** for temporal modeling (Phase 3 Activity 2)
...
# ════════════════════════════════════════════════════════════
code("""# ── Find Swin UNETR pretrained weights ──

OUTPUT_ROOT = Path('/kaggle/working/phase3_swinunetr_outputs')
WEIGHTS_DIR = OUTPUT_ROOT / 'pretrained_weights'
WEIGHTS_DIR.mkdir(parents=True, exist_ok=True)

import glob

def find_swinvit_weights():
    \"\"\"Search for model_swinvit.pt in Kaggle input datasets.\"\"\"
    search_dirs = [
        '/kaggle/input/datasets/boufafamoamed/swinunetr-pretrained-weights',
        '/kaggle/input/swinunetr-pretrained-weights',
        '/kaggle/input/swin-unetr-weights',
        '/kaggle/input/swinunetr-weights',
    ]
    ...
    for f in glob.glob('/kaggle/input/**/*.pt', recursive=True):
        if 'swin' in f.lower() and os.path.getsize(f) > 100_000_000:
            return f
    return None

SWIN_WEIGHTS = find_swinvit_weights()

if SWIN_WEIGHTS:
    print(f'✅ Swin UNETR weights found: {SWIN_WEIGHTS}')
    print(f'   Size: {os.path.getsize(SWIN_WEIGHTS) / 1024**2:.1f} MB')
else:
    print('⚠️ Swin UNETR weights NOT found in Kaggle inputs!')
    print('   Downloading from MONAI...')
    import urllib.request
    url = 'https://github.com/Project-MONAI/MONAI-extra-test-data/releases/download/0.8.1/model_swinvit.pt'
    dst = str(WEIGHTS_DIR / 'model_swinvit.pt')
    try:
        ...
        print('   Please add "swinunetr-pretrained-weights" dataset to this notebook')
        SWIN_WEIGHTS = None""")
...
# ════════════════════════════════════════════════════════════
code("""# ── Swin UNETR model creation + pretrained weight loading ──
# MONAI >= 1.5 removed img_size from SwinUNETR.__init__()
# The model now accepts dynamic input sizes (must be divisible by 32)

import inspect
_swin_sig = inspect.signature(SwinUNETR.__init__)
_has_img_size = 'img_size' in _swin_sig.parameters
print(f'MONAI SwinUNETR API: {"img_size required" if _has_img_size else "dynamic input (no img_size)"}') 

def create_model():
    \"\"\"Create Swin UNETR model (compatible with MONAI 1.3+ and 1.5+).\"\"\"
    kwargs = dict(
    ...
    # For older MONAI that still requires img_size
    if _has_img_size:
...
print(f'Swin UNETR: {n_params:,} params ({n_params/1e6:.1f}M)')

# Load pretrained encoder weights
if SWIN_WEIGHTS:
    print(f'\\nLoading pretrained encoder weights...')
    weight = torch.load(SWIN_WEIGHTS, map_location='cpu', weights_only=True)
    test_model.load_from(weights=weight)
    print(f'✅ Pretrained Swin ViT encoder loaded (BraTS 2021)')
    print(f'   Encoder stages loaded: swinViT (hierarchical transformer)')
    print(f'   Decoder: randomly initialized (will be trained)')
else:
    print('⚠️ No pretrained weights — training from scratch (slower convergence)')
...
def train_fold(fold):
    ...
    best_path = ckpt_dir / f'swinunetr_fold{fold}_best.pth'
    latest_path = ckpt_dir / f'swinunetr_fold{fold}_latest.pth'
    ...
    if SWIN_WEIGHTS:
        ...
        print('  Loaded Swin UNETR pretrained encoder ✅')
    ...
    plt.tight_layout(); plt.savefig(fig_dir / f'swinunetr_fold{fold}_training_curves.png', dpi=150); plt.close()
    
    met_dir = OUTPUT_ROOT / 'metrics'; met_dir.mkdir(parents=True, exist_ok=True)
    with open(met_dir / f'swinunetr_fold{fold}_metrics.json', 'w') as f_out:
...
code("""def extract_embeddings(model, fold):
    ...
    np.savez(emb_dir / f'swinunetr_embeddings_fold{fold}.npz', **embeddings)
    
    meta = {k: {'patient_dir': k.split('__')[0], 'visit': k.split('__')[1]} for k in embeddings}
    with open(emb_dir / f'swinunetr_embeddings_fold{fold}_meta.json', 'w') as f_out:
...
for input_dir in Path('/kaggle/input').iterdir():
    ...
    for pth_file in input_dir.rglob('swinunetr_fold*_best.pth'):
        dest = ckpt_dir / pth_file.name
        if not dest.exists():
            shutil.copy2(str(pth_file), str(dest))
            print(f'  📦 Recovered: {pth_file.name}')
            recovered += 1
    for pth_file in input_dir.rglob('swinunetr_fold*_latest.pth'):
        dest = ckpt_dir / pth_file.name
        if not dest.exists():
            shutil.copy2(str(pth_file), str(dest))
            print(f'  📦 Recovered: {pth_file.name}')
...
target_fold = None
for f in [0, 1, 2]:
    best_p = ckpt_dir / f'swinunetr_fold{f}_best.pth'
    latest_p = ckpt_dir / f'swinunetr_fold{f}_latest.pth'
...
if target_fold is None:
    ...
    for f in [0, 1, 2]:
        emb_path = OUTPUT_ROOT / f'embeddings/swinunetr_embeddings_fold{f}.npz'
        if not emb_path.exists():
            print(f'\\n  Extracting embeddings for fold {f}...')
            model = create_model().to(device)
            best_ckpt = torch.load(ckpt_dir / f'swinunetr_fold{f}_best.pth', map_location=device, weights_only=False)
            model.load_state_dict(best_ckpt['model_state_dict'])
    ...
    for cf in list(completed_folds.keys()):
        emb_chk = OUTPUT_ROOT / f'embeddings/swinunetr_embeddings_fold{cf}.npz'
        if not emb_chk.exists():
            print(f'  Fold {cf} recovered but missing embeddings — extracting now...')
            emb_model = create_model().to(device)
            bc = torch.load(ckpt_dir / f'swinunetr_fold{cf}_best.pth', map_location=device, weights_only=False)
    ...
    best_path = ckpt_dir / f'swinunetr_fold{target_fold}_best.pth'
    if best_path.exists():
...
def visualize_3d_comparison(model, val_loader, fold, n_samples=3):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(val_loader):
            ...
            ax2.set_title('Swin UNETR Prediction', fontsize=13, fontweight='bold')
            for c, (name, color) in enumerate(colors.items()):
            ...
            plt.tight_layout()
            save_path = fig_dir / f'swinunetr_fold{fold}_3d_sample{idx}.png'
...
if vis_fold is not None:
    best_path = ckpt_dir / f'swinunetr_fold{vis_fold}_best.pth'
    if best_path.exists():
...
out_path = _Path(__file__).parent.parent / "notebooks" / "Phase3_A1_SwinUNETR_Training.ipynb"
