...
def upload_weights():
    ...
    try:
        ...
        # nnU-Net v2 (Skipped per request)
        # trainer = MODELS_DIR / "nnU-Net_v2 " / "Dataset002_BRATS19" / "nnUNetTrainer__nnUNetPlans__3d_fullres"
        # if trainer.exists():
        # ...
        
        # SwinUNETR (ViT Phase weights)

        # SwinUNETR (ViT Phase weights)
        swin = MODELS_DIR / "ViT" / "fold1_f48_ep300_4gpu_dice0_9059" / "model.pt"
        if swin.exists():
            shutil.copy2(str(swin), staging / "swinunetr_fold1.pt")
            print(f"  ✅ SwinUNETR Fold 1: {swin.stat().st_size/1e6:.0f} MB")
        else:
            print(f"  ❌ File not found: {swin}")
...
def upload_validation():
    ...
    staging = HDD_STAGING / "brats2024_validation_upload"
    if staging.exists():
    ...
    try:
        ...
        # Hardlinks: removing staging does NOT delete original
        shutil.rmtree(staging, ignore_errors=True)
